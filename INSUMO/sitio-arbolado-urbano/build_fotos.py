...
def procesar(origen: Path, destino: Path, clasificacion: dict, inline: bool,
    ...
    for carpeta in carpetas:
        ...
        for orden, archivo in enumerate(archivos, 1):
            ...
            with Image.open(archivo) as imagen:
                ...
                for nombre, ancho, calidad in TAMANOS:
                    ...
                    # Con --continuar se rehace solo lo que falta o quedó
                    # desactualizado. Convertir 349 fotos de 20 Mpx toma
                    # bastante; no tiene sentido repetirlo entero al agregar una.
                    if continuar and ruta.exists() and \
                            ruta.stat().st_mtime >= archivo.stat().st_mtime:
        ...
        print(f"  {especie:28s} {len(fotos):3d} fotos", flush=True)
...
def main():
    ...
    ap.add_argument("--continuar", action="store_true",
                    help="saltar los derivados ya generados y al día")
    args = ap.parse_args()
    ...
    manifiesto, bundle, entrada, salida, sin_clasificar, por_tipo = procesar(
        origen, destino, clasificacion, args.inline, args.continuar)
